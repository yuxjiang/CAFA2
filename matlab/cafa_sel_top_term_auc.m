function [sel, bsl, info] = cafa_sel_top_term_auc(K, aucs, naive, blast, config)
%CAFA_SEL_TOP_TERM_AUC CAFA select top term-centric AUC
% {{{
%
% [sel, bsl, info] = CAFA_SEL_TOP_TERM_AUC(K, aucs, naive, blast, config);
%
...
% }}}
  ...
  % check inputs {{{
  if nargin ~= 5
    error('cafa_sel_top_term_auc:InputCount', 'Expected 5 inputs.');
  end

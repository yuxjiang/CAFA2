...
%     ----
%     [no knowledge]
%     For a protein to be qualified as type 1, it must have no annotation at all
%     by T0 (among any of the three GO ontologies).
%
%     [limited knowledge]
%     For a protein to be qualified as type 2, it must have no annotation on the
%     one of interest, but have some annotations on the other two ontologies.
%     For example, if protein A is in type 2 benchmark for MFO, is must have no
%     annotation on MFO by T0, but not restrictions on its annotation on BPO or
%     CCO. however, it must have some annotations on either one of the other
%     two, otherwise, it should be in type 1 benchmark.
%
% Dependency
% ----------
%[>]pfp_oabuild.m
...
%[>]pfp_ontbuild.m
  ...
  % check inputs {{{
  if nargin ~= 3
    error('cafa_bm_build_go:InputCount', 'Expected 3 inputs.');
  end

  % gont
  validateattributes(gont, {'struct'}, {'nonempty'}, '', 'gont', 1);

  % raw_goa0
  validateattributes(raw_goa0, {'char'}, {'nonempty'}, '', 'raw_goa0', 2);

  % raw_goa1
  validateattributes(raw_goa1, {'char'}, {'nonempty'}, '', 'raw_goa1', 3);
  % }}}
...
% Last modified: Mon 23 May 2016 06:40:34 PM E
